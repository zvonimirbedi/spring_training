...
public class AspectLogging {
	...
	// com.aop.dao.DAOLoyalty.getDatabaseConnection())")
	// @Before("execution(public void get*())")
	// @Before("execution(void get*())")
	// @Before("execution(* get*())")
	// @Before("execution(* get*(..))")
	/*
	 * @Before("com.aop.aspect.PointcutUtility.pointcutPackageFunction()") private
	 * void beforeGetDatabaseConnection() { theLogger.info("@Before - " + getClass()
	 * + ": loging  execution @Before advice on method"); }
	 */
	...
	public void beforeGetDatabaseConnection(JoinPoint theJoinPoint) {
		...
		for (int i = 0; i < theArguments.length; i++) {
			...
			if (theArguments[i] instanceof Subscription) {
				// theLogger.info("Argument["+i+"] = " + ((Subscription)
				// theArguments[i]).getName());
	...
	@AfterReturning(pointcut = "execution(* com.aop.dao.DAOSubscription.findSubscriptions(..))", returning = "result")
	...
	@AfterThrowing(pointcut = "execution(* com.aop.dao.DAOSubscription.findSubscriptions(..))", throwing = "theException")
	...
	@After("execution(* com.aop.dao.DAOSubscription.findSubscriptions(..))")
	...
	@Around("execution(* com.aop.dao.service.ServiceTraffic.getTraffic(..))")
